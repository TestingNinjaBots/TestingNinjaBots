...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7126250699').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002134020664 -1002177697372').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '7126250699').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL', '-1002221471158')
auth_grp = environ.get('AUTH_GROUP', '-1002177571860')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Autofilter002:Autofilter002@autofilter2.8p69b0e.mongodb.net/?retryWrites=true&w=majority&appName=Autofilter2")
DATABASE_NAME = environ.get('DATABASE_NAME', "Autofilter2")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'Telegram_files')

# Others
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002225933814'))
LAZY_GROUP_LOGS = int(environ.get('LAZY_GROUP_LOGS', '-1002225933814'))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'LazyDeveloper')
P_TTI_SHOW_OFF = is_enabled((environ.get('P_TTI_SHOW_OFF', "True")), False)
IMDB = is_enabled((environ.get('IMDB', "False")), True)
SINGLE_BUTTON = is_enabled((environ.get('SINGLE_BUTTON', "True")), False)
CUSTOM_FILE_CAPTION = environ.get("CUSTOM_FILE_CAPTION", "⚡<b>File uploaded by [DH Networks™](https://t.me/DH_Networks)</b>⚡\n\n📥 <b>File Name: </b> ➥  {file_caption} \n💎 <b>Size: </b><i>{file_size}</i>\n\n                ❤️<b>WE LOVE YOU</b>❤️\n🔥  ↭ <b>Join Now [MoviesAdda™](https://t.me/real_MoviesAdda3)</b> ↭  🔥")
BATCH_FILE_CAPTION = environ.get("BATCH_FILE_CAPTION", CUSTOM_FILE_CAPTION)
IMDB_TEMPLATE = environ.get("IMDB_TEMPLATE", "<b>File Name: {query}</b> \n‌‌‌‌IMDb Data by: @Sablinkhere \n\n🏷 Title: <a href={url}>{title}</a>\n🎭 Genres: {genres}\n📆 Year: <a href={url}/releaseinfo>{year}</a>\n🌟 Rating: <a href={url}/ratings>{rating}</a> / 10 \n\n♥️ we are nothing without you ♥️ \n💝 Please Share Us 💪\n\n𝑫𝑶𝑾𝑵𝑳𝑶𝑨𝑫 𝑨𝑳𝑳 𝑭𝑰𝑳𝑬𝑺✅ ")
...
lazy_renamers = [int(lazrenamers) if id_pattern.search(lazrenamers) else lazrenamers for lazrenamers in environ.get('LAZY_RENAMERS', '1744264999').split()]
LAZY_RENAMERS = (lazy_renamers + ADMINS)if lazy_renamers else []
REQ_CHANNEL = int(environ.get('REQ_CHANNEL', '-1002225933814'))
...
my_users = [int(my_users) if id_pattern.search(my_users) else my_users for my_users in environ.get('MY_USERS', '6785693880').split()]
...
OUO_IO_API_KEY = ""
MAX_MESSAGE_LEFal = 4096
PROCESS_MAX_TIMEOUT = 0
DEF_WATER_MARK_FILE = ""
LOGGER = logging
lazydownloaders = [int(lazydownloaders) if id_pattern.search(lazydownloaders) else lazydownloaders for lazydownloaders in environ.get('PRIME_DOWNLOADERS', '1744264999').split()]
...
CAPTION_BUTTON_URL = "https://t.me/sablinkhere"
